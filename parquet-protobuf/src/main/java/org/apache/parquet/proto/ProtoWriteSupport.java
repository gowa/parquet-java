...
public class ProtoWriteSupport<T extends MessageOrBuilder> extends WriteSupport<T> {
  ...
  class BinaryMessageWriter extends FieldWriter {
    ...
    @Override
    final void writeRawValue(Object value) {
      recordConsumer.addBinary(convertMessageOrBuilderToBinary((MessageOrBuilder) value));
    }
  }

  static Binary convertMessageOrBuilderToBinary(MessageOrBuilder value) {
  ...
  static class ResolvedReflection {
    ...
    static class ByteBuddyProto3FastMessageWriterMethods {
      ...
      final Method enumNameNumberPairs = ReflectionUtil.getDeclaredMethod(ByteBuddyProto3FastMessageWriters.ByteBuddyProto3FastMessageWriter.class, "enumNameNumberPairs", String.class);
      final Method writeNotOptimizedMessageFieldRawValue = ReflectionUtil.getDeclaredMethod(ByteBuddyProto3FastMessageWriters.ByteBuddyProto3FastMessageWriter.class, "writeNotOptimizedMessageFieldRawValue", int.class, MessageOrBuilder.class);
  ...
  static class ReflectionUtil {
    ...
    static class ArrayReflectionType implements java.lang.reflect.Type {
      ...
      public boolean equals(Object o) {
        ...
        ArrayReflectionType that = (ArrayReflectionType) o;
  ...
  interface OptimizedWriter {
    Optional<FastMessageWriter> getFastMessageWriter(ProtoWriteSupport<?>.MessageWriter messageWriter);
  }
  ...
  static class ByteBuddyProto3FastMessageWriters extends Proto3FastMessageWriters {
    ...
    abstract static class ByteBuddyProto3FastMessageWriter implements OptimizedWriter {
      ...
      ByteBuddyProto3FastMessageWriter(ProtoWriteSupport<?> protoWriteSupport, ProtoWriteSupport<?>.MessageWriter rootMessageWriter) {
        ...
        indexFieldWriters(rootMessageWriter, notOptimizedFieldWriters, optimizedFieldWriters);
      ...
      public Optional<FastMessageWriter> getFastMessageWriter(ProtoWriteSupport<?>.MessageWriter messageWriter) {
        int methodIdx = optimizedFieldWriters.getOrDefault(messageWriter, -1);
        if (methodIdx < 0) {
          return Optional.empty();
        }

        Class<?> messageClass = messageWriter.getFieldReflectionType();
        Class<?> messageOrBuilderInterface = ReflectionUtil.getProto3MessageOrBuilderInterface(messageClass).get();
        ...
        try {
          Consumer<MessageOrBuilder> consumer = (Consumer<MessageOrBuilder>) LambdaMetafactory.metafactory(lookup,
              ...
              lookup.unreflect(ReflectionUtil.getDeclaredMethod(this.getClass(), "writeAllFields$" + methodIdx, messageOrBuilderInterface)),
              MethodType.methodType(void.class, messageOrBuilderInterface)
          ).getTarget().bindTo(this).invokeExact();

          return Optional.of(new FastMessageWriter() {
            @Override
      ...
      void writeNotOptimizedMessageFieldRawValue(int fieldWriter, MessageOrBuilder value) {
        notOptimizedFieldWriters[fieldWriter].writeRawValue(value);
      }

      void writeBinaryMessageFieldRawValue(MessageOrBuilder value) {
        getRecordConsumer().addBinary(convertMessageOrBuilderToBinary(value));
      }

      private static void indexFieldWriters(ProtoWriteSupport<?>.MessageWriter rootMessageWriter,
...
//       }
    ...
    interface MessageWriterVisitor {
      final class FieldPath {
        ...
        FieldPath pop() {
          if (isRoot()) {
            ...
            FieldPath r = new FieldPath();
            r.path.addAll(this.path.subList(0, this.path.size() - 1));
            return r;
          }
        }
      ...
      class RegularField<T extends ProtoWriteSupport<?>.FieldWriter> extends Field<T> {
        ...
        @Override
        java.lang.reflect.Type getFieldReflectionType() {
          return rawValueWriter().getFieldReflectionType();
        }
        ...
        Optional<? extends Class<? extends MessageOrBuilder>>
        getFieldReflectionTypeMessageOrBuilderInterface() {
          if (!isMessage() && !isBinaryMessage()) {
            throw new IllegalStateException("illegal to call on non-message field");
          }
          return ReflectionUtil.getMessageOrBuilderInterface(
              (Class<?>) rawValueWriter().getFieldReflectionType());
        }
      ...
      class MapField extends Field<ProtoWriteSupport<?>.MapWriter> {
        ...
        @Override
        java.lang.reflect.Type getFieldReflectionType() {
          return fieldWriter.getFieldReflectionType();
        }
    ...
    */
    ...
    static class FastMessageWriterImplementation {
      ...
      final Map<MessageWriterVisitor.MethodKey, Integer> notOptimizedMessageFieldWriters = new LinkedHashMap<>();
        ...
        MessageWriterVisitor.traverse(start, new MessageWriterVisitor() {
          ...
          public boolean visitMessageWriter(RegularField<?> field) {
            ...
            if (!field.getFieldReflectionTypeProto3MessageOrBuilderInterface()
                .isPresent()) {
              if (!notOptimizedMessageFieldWriters.containsKey(field.asMessageWriterMethodKey())) {
                notOptimizedMessageFieldWriters.put(field.asMessageWriterMethodKey(), notOptimizedMessageFieldWriters.size());
              }
            }
            return true;
          }
...
*/
        ...
        MessageWriterVisitor.traverse(start, new MessageWriterVisitor() {
          @Override
          public boolean visitMessageWriter(RegularField<?> field) {
            if (definedMethods.add(field.asMessageWriterMethodKey())) {
              classBuilder[0] = classBuilder[0].define(fieldPathToMethodDescriptionMap.get(field.asMessageWriterMethodKey()))
      ...
      class WriteAllFieldsForMapEntryImplementation extends FastMessageWriterMethodBase {
        ...
        WriteAllFieldsForMapEntryImplementation(MessageWriterVisitor.MapField field) {
          ...
          try (LocalVar thisLocalVar = localVars.register(classBuilder[0].toTypeDescription())) {
            try (LocalVar key = localVars.register((Class<?>) field.key().getFieldReflectionType())) {
              try (LocalVar value = localVars.register(field.value().isEnum() ? int.class : (Class<?>) field.value().getFieldReflectionType())) {
        ...
        Implementation writeFromVar(MessageWriterVisitor.RegularField<?> field, LocalVar val, LocalVar recordConsumer) {
          if (field.isEnum()) {
            ...
            return new BinaryMessageFieldWriter(field, recordConsumer).writeFromVar(val);
          }
      ...
      abstract class FastMessageWriterMethodBase implements Implementation {
        ...
        protected void writeMessageFields(
            MessageWriterVisitor.RegularField<? extends ProtoWriteSupport<?>.MessageWriter> regularField) {
          Class<? extends MessageOrBuilder> messageOrBuilderInterface = regularField
              .getFieldReflectionTypeMessageOrBuilderInterface()
              .get();
...
//           }
      ...
      private void registerWriteAllFieldsForMessageMethod(MessageWriterVisitor.RegularField<?> field) {
        ...
        if (!fieldPathToMethodNameMap.containsKey(key)) {
          ...
          fieldPathToMethodNameMap.put(key, methodName);

          MethodDescription.Token token = new MethodDescription.Token(
              methodName,
              Visibility.PUBLIC.getMask(),
              ForLoadedType.of(void.class),
              Arrays.asList(ForLoadedType.of(field.getFieldReflectionTypeMessageOrBuilderInterface()
                  .get())));

          fieldPathToMethodDescriptionMap.put(key, new MethodDescription.Latent(classBuilder[0].toTypeDescription(), token));
        }
      }
